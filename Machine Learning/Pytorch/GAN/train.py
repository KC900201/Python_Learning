...
fixed_noise = torch.randnn(32, Z_DIM, 1, 1).to(device)
...
for epoch in range(NUM_EPOCHS):
    for batch_idx, (real, _) in enumerate(loader):
        real = real.to(device)
        noise = torch.randnn((BATCH_SIZE, Z_DIM, 1, 1)).to(device)

#       Train Disciminator max log(D(x)) + log(1 - D(G(z))
        disc_real = disc(real).reshape(-1) # N x 1 x 1
        loss_disc_real = criterion(disc_rea, torch.ones_like(disc_real))
